<?php
if ( isset($catAddID) ) :
    $catState = getCatInMyLessons($catAddID);    
?>
<?php if ($catState==1) {?>
        <!--done-->
...
<?php } else { ?>                
        <!-- remove -->    
         <a style="display:none;" id="btn_rm_<?php echo $catAddID;?>" href="javascript:void(0);" onclick="" title="<?php _e('remove from my lessons', 'swgeula');?>" alt="<?php _e('remove from my lessons', 'swgeula');?>" class="add_lesson_btn remove"   onMouseOut="this.style.color='<?php echo $color; ?>'" style="color:<?php echo $color; ?>">
            <i class="fa fa-star add"></i>
            <i class="fa fa-times remove"></i>
        </a>
        <!-- add -->    
